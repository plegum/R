...
################################################################################
GR <- function( XT,XBY,XWHAT,FUN) {
  if ( missing(FUN) || missing(XWHAT) ) {
    XT <- XT[,.N,by=XBY]
  } else {
  XT$XBY <- XT[[XBY]]
  XT$XWHAT <- XT[[XWHAT]]
  XT <- na.omit(XT[,.( XBY,XWHAT )])
  XT <- XT[,ROUND(as.double(FUN( XWHAT )),2),by=XBY]
  colnames(XT) <-c(XBY,XWHAT)
  }
XT
}
################################################################################
OTODOM_MAP <- function(XM,
Y_FILL='VAL',
Y_TRANS="identity",
YV_COLORS=c( rev(viridis::viridis(100) ))
) {
if (Y_TRANS == "log1p"  ) {
